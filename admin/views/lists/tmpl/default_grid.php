...
quickcontentLoader('admin://class/grid');
...
foreach( $this->items as $k => $item ):
	...
	$canCreate	= $user->authorise('core.create',		'com_quickcontent');
	$canEdit	= $user->authorise('core.edit',			'com_quickcontent');
	$canCheckin	= $user->authorise('core.manage',		'com_quickcontent');
	$canChange	= $user->authorise('core.edit.state',	'com_quickcontent');
	$canEditOwn = $user->authorise('core.edit.own',		'com_quickcontent');
	...
	// =================================================================================
		...
		if ($item->get('a_checked_out'))
			$content .= JHtml::_('jgrid.checkedout', $i, $item->get('a_checked_out'), $item->get('a_checked_out_time'), 'lists.', $canCheckin);
		
		if ($canEdit || $canEditOwn) 
			$content .= JHtml::link(JRoute::_('index.php?option=com_quickcontent&task=list.edit&id='.$item->a_id), $item->get('a_title')) ;
		
		
		$content .= '<div class="small">'.JText::sprintf('JGLOBAL_LIST_ALIAS', $this->escape( $item->get('a_alias') )).'</div></div>' ;
		
		JHtml::_('dropdown.edit', $item->id, 'lists.');
	...
	// =================================================================================
		...
		$content = JHtml::_('jgrid.published', $item->a_published, $i, 'lists.', $canChange, 'cb', $item->a_publish_up, $item->a_publish_down);
